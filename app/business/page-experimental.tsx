  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#EDEDED] overflow-x-hidden">
      ...
      <section className={`${submitted ? "pt-16 sm:pt-24" : "min-h-[80vh] flex"} flex-col items-center justify-center px-4 pt-12 sm:pt-0 pb-8 sm:pb-0 relative overflow-hidden transition-all duration-700`}>
        ...
        {/* Stage photo — top half only, sets the mood */}
        {!submitted && (
          <div className={`absolute top-0 left-0 right-0 h-[55%] transition-opacity duration-[2500ms] ${entered ? "opacity-100" : "opacity-0"}`}>
            <img
              src="/space5.png"
              alt=""
              className="absolute inset-0 w-full h-full object-cover"
              style={{ objectPosition: "50% 65%", filter: "grayscale(0.7) contrast(1.15) brightness(0.85)" }}
            />
            {/* Grain */}
            ...
            {/* Bottom fade — hard dissolve to page bg */}
            <div className="absolute bottom-0 left-0 right-0 h-[50%]" style={{ background: "linear-gradient(to top, #0a0a0a 0%, #0a0a0a 10%, transparent 100%)" }} />
            {/* Top fade — hide ceiling */}
            <div className="absolute top-0 left-0 right-0 h-[30%]" style={{ background: "linear-gradient(to bottom, #0a0a0a 0%, #0a0a0a 20%, transparent 100%)" }} />
            {/* Side vignette */}
            <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 70% at 50% 50%, transparent 20%, rgba(10,10,10,0.5) 60%, rgba(10,10,10,0.9) 90%)" }} />
          </div>
        )}
